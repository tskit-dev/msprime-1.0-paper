...
% JK: commenting this out for now as we're covering it in the Mutations
% section.
...
tree in Newick format. Population genetic simulators have tended to
...
The output of the combined simulations of ancestry and mutations is
then some representation of the resulting nucleotide sequences
(or perhaps some derived statistics of those sequences);
information about the mutations themselves is not typically
provided to the user.

The approach taken in \msprime\ and \tskit\ is quite different.
As illustrated in Fig.~\ref{fig-ts-example}, mutations are an
integral part of the tree sequence data model, and genetic
variation is encoded by sites and the mutations at those sites.
This may seem at first inconvenient, but is extremely powerful.
...
we required $O(nm)$ space to store the sequence data as a variant
matrix. However, if this simulation was of a recombining genome
with $t$ trees, then the \tskit\ tree sequence encoding
requires $O(n + t + m)$ space~\citep{kelleher2016efficient}. For
...
is several orders of magnitude faster for large
sample sizes~\citep{ralph2020efficiently}.

The advantages of storing mutations rather than sequences have
recently become clear in the context of vast genomic
datasets produced during the SARS-CoV-2 pandemic.
\cite{turakhia2021ultrafast} propose the
Mutation Annotated Tree (MAT)
format~\citep{mcbroome2021daily}---consisting of a Newick tree
and associated mutations in a binary format---as an efficient way to store
and process large viral datasets,
and achieve excellent compression and processing performance.
Similarly, \texttt{phastsim}~\citep{demaio2021phastsim}
was developed to simulate sequence evolution on such large SARS-CoV-2
phylogenies, and also outputs a tree annotated with mutations (not
in MAT format, however) to avoid the bottleneck of generating
and storing the simulated sequences.
While these methods illustate the advantages of the general
approach of storing ancestry and mutations over sequences,
...
Sites and mutations are fully integrated into the tree sequence data
model and the \tskit\ library.
As illustrated in Fig~\ref{fig-ts-example},
each site corresponds to some position
along the genome, and each mutation occurs at a given site, above
a particular node, with a given derived state (along with other
information, such as the time the mutation occured).
% Now talk about why tskit allows us to easily generate mutations
% on any input tree sequence topology.


\jkcomment{stuff to cover somewhere in this section}
\begin{itemize}
\item Mutation simulations is only included in msprime for historical reasons.
It would be better to see msprime's mutation simulation as a separate
package. The mutation and ancestry code in msprime have no priveledged
access to each other, and mutation simulation would be no less efficient
if done in a different package.
\item Really emphasise this point: you can use msprime's mutation simulation
on \emph{any} tskit tree sequence, produced by SLiM, fwdpp, etc or
even tsinfer. It's completely modular.
\end{itemize}
