...
Because we are usually simulating neutral evolution in coalescent simulations,
the problem of simulating generating variation can be decomposed into
two independent steps:
first simulating the genetic ancestry (the trees), and then simulating
variation by superimposing neutral mutation processes on those trees.
The classical \texttt{Seq-Gen}~\citep{rambaut1997seq} program allows a range
of different models of sequence evolution to be imposed on a given
...
but in several cases including a range of different
models of sequence evolution
built-in~\citep[e.g.][]{mailund2005coasim,excoffier2011fastsimcoal}.
The ultimate output of the simulations of trees and mutations is
then some representation of the resulting nucleotide sequences,
and information about the simulated mutations is lost.

\jkcomment{stuff to cover somewhere in this section}
\begin{itemize}
\item Tskit's approach of storing the mutations is a very simple idea,
but has profound computational consequences. We can compute statistics
about the sequences \emph{without ever seeing those sequences}.
