...
% Seems somewhat unfair to single out Iain and Aylwyn here, but a systematic
% review would be long and tedious, and I'm sure they won't be too upset about
% it.
...
essentially mean a representation of genetic ancestry as a graph; for example,
...
helpful. One may wonder if a more generic term such as ``genetic pedigree''
would be a more useful description, in such cases where the precise form of the
encoding is not important. \jkcomment{Genetic pedigree is probably
just as confusing, on reflection. We can either come up with a different
term, or delete this sentence.}
...
thornton2014cpp,kelleher2016efficient,becheler2019quetzal,haller2019slim}. This
approach greatly increases the flexibility of simulations, and allows all
processing for many simulation analyses to be performed in a single script. The
primary interface for \msprime\ is through a well documented and stable Python
API, which has allowed a number of downstream tools to be built on
top of
% Found these through the Dependent packages on GitHub
it~\citep{kamm2020efficiently,adrion2020predicting,adrion2020community,
rivera2021simulation,de2021geonomics}
as well as numerous easily reproducible simulation
% TODO: more papers with exemplorary reproducible simulation pipelines based
% on msprime!
pipelines~\citep[e.g.][]{martin2017human}.
We also provide an \ms\ compatible
command line interface to support existing workflows, as well as the
\texttt{msp simulate} command that supports specifying demographic models in
Demes format~\citep{gower2021demes}.

A major change for the 1.0 release is the introduction of a new set of APIs,
