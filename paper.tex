...
Motivated by the large numbers of SARS-CoV-2 genomes currently
available, \cite{turakhia2021ultrafast} propose a similar approach
to encoding variation data for non-recombining organisms,
where a Newick tree is annotated by mutations~\citep{mcbroome2021daily}.
Similarly, the Phastsim~\citep{demaio2021phastsim} mutation simulator,
developed primarily to support simulation of SARS-CoV-2 trees
with hundreds of thousands of leaves, can optionally output a
Newick tree, where nodes are annotated with the mutations.
However, the dependence on the Newick format for encoding
topological information means that it suffers from all the
problems of newick [what?], and there is no software library support
[confirm].
