...
% First authors
%% FIXME affiliations are all broken
...
% Franz
\affil[1]{Mathematical and Computational Population Genetics, University of T\"ubingen}
% Gertjan, Konrad
\affil[2]{Institute of Evolutionary Biology, The University of Edinburgh}
% Daniel
\affil[3]{FIXME: DANIEL'S AFFILIATION}
% Graham
\affil[4]{Lundbeck GeoGenetics Centre, Globe Institute, University of Copenhagen}
% Simon, Dom, Ivan
\affil[5]{FIXME AARON's AFFILIATION}
% Georgia
...
% Castedo
\affil[9]{FIXME: CASTEDO's AFFILIATION}
% Jared, Murillo, Peter, Andy
\affil[10]{Department of Biology and Institute of Ecology and Evolution,
    University of Oregon}
% Ariella
\affil[11]{Department of Genetics, University of North Carolina at Chapel Hill}
% Gregor
\affil[12]{The Roslin Institute and Royal (Dick) School of Veterinary Studies,
    University of Edinburgh}
% Bing
\affil[13]{FIXME: BING's AFFILIATION}
% Winni
\affil[14]{FIXME: WINNI's AFFILIATION}
% Simon, Dom, Ivan
\affil[15]{Department of Human Genetics, McGill University}
% Michael
\affil[16]{FIXME: MICHAEL's AFFILIATION}
% Nathaniel
\affil[17]{FIXME: NATHANIEL's AFFILIATION}
% Consuelo
\affil[18]{National Laboratory of Genomics for Biodiversity (LANGEBIO),
    Unit of Advanced Genomics, CINVESTAV, Irapuato, Mexico}
% Saunack
\affil[19]{FIXME: SAUNACK's AFFILIATION}
% Thibaut
\affil[20]{Professorship for Population Genetics, Department of Life Science Systems, Technical University of Munich}
% Kevin
\affil[21]{Ecology and Evolutionary Biology, University of California at Irvine}
% Hugo
\affil[22]{FIXME HUGO's AFFILIATION}
% Jere
...
structure. Moreover, development of \msprime\ is collaborative
and open source, which reduces duplication of effort and promotes
...
\adkcomment{i found using 'single chromosome strand' here
and then later 'haplotype' confusing, so edited}
to a single chromosome, associated with its birth time
and other optional information. For instance, a node may be
associated with an \emph{individual}, so that, a diploid
individual is composed of two separate nodes (we omit
the individual table in Fig.~\ref{fig-ts-example} as it is optional).
While nodes define chromosomal sequences that exist at a specific time,
edges define genetic inheritance between these nodes. 
An \emph{edge} contains information about the
parent node, the child node and the left and right coordinates
...
history as nodes and edges in a tree sequence (see the \nameref{sec-ts} section),
...
thus we believe \msprime\ to be the highest performing
coalescent simulator that allows for sweeps currently.
% JK: do we need an extra proviso like this?
% and this is unlikely to change unless sweeps dominate the ancestral process.
