...
information about the mutations that have been simulated
...
This may seem inconvenient at first, but it is extremely powerful.
...
recently become clear in the context of the vast genomic
...
associated \texttt{matUtils} program as an efficient way to store
...
The \tskit\ library provides first-class support for site and mutation
data, fully integrated with the topological information encoded in the
node and edge tables (Fig~\ref{fig-ts-example}). The mature and
...
contains code to simulate mutations: the process could (and arguably should)
be performed in a separate package, and there would be no performance
penalty for doing so. It is important to note that \msprime's
...
Fig.~\ref{fig-mutations-perf} shows the time requried to generate
...
on an edge-by-edge (see Fig.\ref{fig-ts-example}) basis, rather than
...
Even when we consider a single tree, \msprime\ is far more
efficient than methods such as \SeqGen\ and \Pyvolve,
and has similar performance to \phastsim.
