...
% Peter L. Ralph
% Institute of Ecology and Evolution, Departments of Mathematics and Biology, University of Oregon, USA


% \author[n]{Graham Gower}
% \affil[n]{Lundbeck GeoGenetics Centre, Globe Institute, Faculty of Health and Medical Sciences, University of Copenhagen, Denmark}
...
we know that the number of ARG nodes is bounded by
$\rho^2 \log^2 n$ for
scaled recombination rate $\rho$ and sample size $n$
(see the \nameref{sec-recombination} section),
whereas the size of the tree sequence encoding is
$O(n + \rho \log n)$~\citep{kelleher2016efficient}. This difference between a
quadratic and a linear dependency on $\rho$ is profound, and shows why ARG
based approaches are not suitable for large scale simulation.
