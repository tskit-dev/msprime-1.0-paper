...
and simulations by \ms, \texttt{msHot} and \SLiM.
...
of the two linages, but not the most recent common genetic ancestor
...
large values of $\rho$, and so it seems a tighter bound
than Eq.~\eqref{eqn-hsw-bound} would
be required to accurately predict simulation time for a
given $\rho$ and $n$.
...
may not be feasible. For example, for
\emph{Drosophila melanogaster} chromosome 2L, with
$\rho\approx 3.9\times10^6$, Fig.~\ref{fig-ancestry-perf}
predicts that simulation would require around 177 hours. For such
large values of $\rho$  we recommend
...
% JK: why? ^
% I'm worried people will take this as meaning "run every possible
% combination of parameters". I think the genome length should be basically
% independent of everything else, right?
...
many (roughly half) of these lineages have at
least two distant segments of ancestral material
separated by a long segment of genome, implying that $\ell \approx L / 4$.
Then, since the rates of recombination and common ancestor events
are equal the time the maximum number of lineages is present,
we see that $M$ is roughly equal to $L N_e$.
...
\end{center}
\caption{\label{fig-selection-perf}Comparison of the run time for
