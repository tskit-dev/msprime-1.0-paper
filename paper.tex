...
genealogical tree relating the samples to each other.
...
The second development reducing the utility of many simulators is the
...
resolving a major source of confusion and error. The previous APIs (like \texttt{simulate}) are fully
...
or creating forked versions (i.e., independent copies) of existing
...
identity by descent as a set of samples having approximately the same time
...
Just as recombination, gene conversion events split how genetic material
...
which can be inverted to obtain the ``inverse instanteous coalescence rate'',
or IICR of \citet{chikhi2018inverse}.
...
representation of the stochastic process that describes the coalescent with recombination. This
...
genetic ancestries conditioned on an existing partial
...
on code quality and correctness. As in any large code base,
...
% Simulation software is historically a total shitshow

Simulation is a vital tool in population genetics, but the approach to
developing simulation software has historically been dysfunctional. A huge
proliferation of tools have been published (the references here are not
exhaustive) and only a small minority of these are actively developed and
...
development. Although developing a simulator is certainly a learning
opportunity, it is also an opportunity to learn poor development practices. A
large number of unmaintained tools with mostly overlapping features represents
a profound waste of effort, and it is worth considering what could have been
achieved if even a small fraction of this effort had been invested in shared
...
realisation across the sciences [citations] that investing in
...
Most contributors would agree that this is also a learning experience,
allowing them to improve their software development skills in a
welcoming community environment and being exposed to
state-of-the-art infrastructure. We hope that users who find that
...
\jkcomment{TODO either need another paragraph linking these up, or we just
 drop this last one?}
...
have made it possible to estimate such ancestry at scale for
...
As we have seen, \msprime\---through the succinct tree sequence
data structure and \tskit\ library---can provide simulated
genetic ancestry at any scale or degree of detail required, and should
provide a valuable source of ground-truth for evaluating
these methods.
