...
% Bing
\affil[14]{FIXME: BING's AFFILIATION}
% Winni
\affil[15]{FIXME: WINNI's AFFILIATION}
% Simon, Dom, Ivan
...
% Saunack
\affil[20]{FIXME: SAUNACK's AFFILIATION}
% Thibaut
...
% Hugo
\affil[23]{FIXME HUGO's AFFILIATION}
% Jere
...
% Encoding ancestry
Before we define tree sequences we require so additional terminology.
Let a ``ploid'' be the
% JK this isn't right either, but it's a start.
% We should add a sentence clarifying what an individual is here.
% FIXME
complete set of genetic material that a child inherits from a parent,
so that, e.g., each human has two ploids~\citep{jouganous2017inferring}.
Then, given a set of sample ploids
the structure
of their genetic ancestry is described by a set of \emph{nodes}
and \emph{edges} (Fig.~\ref{fig-ts-example}).
A node in tree sequence terminology corresponds to a ploid,
associated with its birth time
and other optional information.
% JK: Now that we have ploids, maybe it doesn't help to talk about
...
Edges then define the genetic inheritence between nodes.
...
ploids separated by many generations.
...
% Do we include this sentence? It's true, I'm sure: someone new to the field reads
% Iain and Aylwyn's paper, and goes, hmm, this ARG thing sounds interesting,
% let's follow up on that Griffiths reference... Maybe this is part of the
% reason why ARGs have such a fearsome reputation for being difficult:
% there's a basic confusion about what they actually *are*.
