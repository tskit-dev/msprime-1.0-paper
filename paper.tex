...
The term Ancestral Recombination Graph (ARG) was introduced by
...
understanding of the recombination process. In Griffiths' ARG formulation, we
regard a realisation of the coalescent with recombination as a graph in which
nodes represent common ancestor or recombination events, and edges represent
lineages through which ancestral material flows. We may consider the ``big''
ARG, in which we wait an exponentially large time until all sampled lineages
coalescence into a single ancestor~\citep{ethier1990two}, or the ``little'' ARG
in which we wait until all marginal trees have fully coalescenced (like
Hudson's algorithm).

% Seems somewhat unfair to single out Iain and Alywyn here, but a systematic
% review would be long and tedious, and I'm sure they won't be too upset about it.
...
essentially mean a record of genetic ancestry as a graph; for example,
\cite{mathieson2020ancestry} informally define ARGs as the subset of an
individual's pedigree from which they have inherited ancestral material. Such
imprecision is unhelpful, since there are any number of graphical encodings of
genetic ancestry, and referring to the original Griffiths papers does little
to clarify. As we shall see, there are situations in which the precise form of the
graph is important, and conflating high-level representation of genetic
ancestry with this precise mathematical object is not helpful.
One may wonder if a more generic term such as ``genetic pedigree''
would be a more useful description, in such cases where the precise form
of the encoding is not important.

% ~\cite[.e.g.][]{minichiello2007mapping}
% \cite{minichiello2006mapping} are somewhat more explicit
% "An ARG provides a record of all coalescence and recombination events since the
% divergence of the sequences under study and specifies a complete genealogy at
% each genomic position
% \citep{rasmussen2014genome}
For our purposes here, an ARG is a realisation of the coalescent with
recombination, in the Griffiths sense.
As described in detail by~\cite{kelleher2016efficient}, Hudson's algorithm
essentially works by dynamically generating and traversing a little ARG.
...
This approach is one of the key reasons for \msprime's efficiency.
...
and the ancestral haplotypes on which coalescences occured, information
is lost in this mapping down from ARG space. In particular, we lose
information about individuals who were common ancestors but where
no coalescences occured and also information about the precise time
...
but there are occasions in which is useful. We therefore provide
the \texttt{record\_full\_arg} option in \msprime\
to store a representation of the compete ARG traversed during simulation.
This is done by storing extra nodes (marked with specific flags, so they
can be easily identified later) in the tree sequence.
Fig.~\ref{fig-recombination-perf}B shows that these extra ARG nodes
and edges quickly come to dominate the output as we increase the sequence length,
