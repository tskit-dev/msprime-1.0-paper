...
inference methods~\citep{griffiths1994sampling, birkner2009modified,galtier2000detecting,
...
See the Multiple Merger Coalescent Model appendix for more details and model derivations.
...
For details, see the appendix: Likelihood Calculations.
...
fixation~\citep{coop2004ancestral}, as detailed in the Selective Sweeps Model
...
AG was supported by NIH award R00HG008696 to Daniel R Schrider.
...
PLR was supported by NIH award R01HG010774.
...
with probability $1-\beta/\alpha$,
and randomly choose a different nucleotide otherwise.
This produces the correct distribution by Poisson thinning:
a Poisson process with rate $\alpha$ in which each point is kept independently
with probability $\beta / \alpha$ is equivalent to a Poisson
...
As discussed in the Recombination section, the time complexity of
...
increments $\delta t$. Then, given the frequency $p$ at time $t$ ago,
the frequency $p^\prime$ at time $t + \delta t$ ago is given by
\begin{equation*}
p^\prime =
\begin{cases}
