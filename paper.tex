...
% I am a bit confused with the following lines - as the section in the main paper says, algo is done on edges,
% but this is skipped here and the way its written it makes it confusing - #edges are propto #marginal_trees
% so there should be no difference (the way I read this part), but in fact the difference on working with edges
% or each tree and its branches can be large
...
% I don't understand what is meant by "older mutations are considered first"?
% Aren't past/older mutations irrelevant given the current state (=parental allele) and a transition matrix?
...
% What is meant by "some randomness"?
and some randomness.
...
% which algo do you refer above? InfiniteAlleles or the general version above the above paragraph? Better to spell it out.
...
% I think I understand what you want to say here, but it would be clearer if an example would have been given - similar to the next section.
